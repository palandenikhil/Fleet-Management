  ...
  const handleCarSelection = (carId) => {
    setSelectedCars((prevSelectedCars) =>
      prevSelectedCars.includes(carId)
        ? prevSelectedCars.filter((id) => id !== carId) // Deselect if already selected
        : [...prevSelectedCars, carId] // Select if not selected
    );
  };

  // Handle Done Button Click
  const handleDone = () => {
    const details = availableCars.filter((car) => selectedCars.includes(car.carId));
    setSelectedCarDetails(details);
  };

  // Handle Cancel Button Click
  const handleCancel = () => {
    window.location.href = "/bookingdetails"; // Redirect to the booking page
  };
