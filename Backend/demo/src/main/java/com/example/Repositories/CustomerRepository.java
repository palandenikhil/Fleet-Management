...
public interface CustomerRepository extends JpaRepository<CustomerMaster,Integer>{
	...
	CustomerMaster getCustomerByEmialId(@Param("email") String email);
	
	@Query(value = "SELECT * FROM customer_master WHERE customerId = :id", nativeQuery = true)
	CustomerMaster getCustomerById(@Param("id") int id);
	
	@Transactional
    @Modifying
    @Query(value = "UPDATE customer_master SET first_name = :firstName, last_name = :lastName, email = :email WHERE customerId = :id", nativeQuery = true)
    void updateCustomer(@Param("id") int id, @Param("firstName") String firstName, @Param("lastName") String lastName, @Param("email") String email);
	 ...
	 @Transactional
	    @Modifying
	    @Query(value = "DELETE FROM customer_master WHERE customerId = :id", nativeQuery = true)
